# pOkeCR
### Description:
The project aims to streamline the proccess of Pokémon nuzlocke calcing for Pokémon Fire Red by fetching all of the required data off of the screen. 

Inside the run_and_bun file is code, which calls the main script (located in display.py) through the press of the F6 button. Running the code using a hotkey is important, as the whole screen needs to be visible during the proccess. A custom OCR was built for this project, as existing libraries (e.g. Tesseract) struggled to detect pixelated Pokémon font.

The main function then runs the fetch_pokemon_data, fetch_moves_and_data, create_move_list and calc_hp_speed functions.
